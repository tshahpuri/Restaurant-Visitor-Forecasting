---
title: 'Recruit Restaurant Visitor Prediction'
date: '`r Sys.Date()`'
output:
    html_notebook
...
if (on_kaggle == 0){
  rpath <- "recruit-restaurant-visitor-forecasting/"
  wpath <- "rrv-weather-data/"
  wdpath <- "rrv-weather-data/1-1-16_5-31-17_Weather/"
} else {
...
We need to make some changes to the date format for exploration purposes.
...
First, we'll look at the distribution of features in the individual data files. This initial visualization will be the basis for our build analysis.


## Air Visits

We start with the number of visits to the *air* restaurants. We plotted the total number of daily visitors over the entire * training * time frame, as well as the average number of weekly, monthly and daily visitors
...
airp3 <- air_visits %>%
  ...
  geom_col() +theme(legend.position = "none", axis.text.x  = element_text(angle=45, hjust=1, vjust=0.9)) +
